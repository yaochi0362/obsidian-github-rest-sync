...
- **Empty folders are never synced, so a new one is never mistaken for an abandoned one**: git has no concept of an empty folder at all - one can only ever exist there because some file's path happens to be inside it. Creating a folder doesn't trigger a sync by itself; it starts existing for sync purposes the moment you add a real file inside it, at which point that file's own create event syncs normally and the folder comes along for free as part of its path. This sidesteps guessing "is this folder new or old" entirely, with nothing written into the folder and no race with Obsidian's own new-folder/rename flow
- **No leftover empty folders from a delete**: deleting a file doesn't remove its now-empty parent folder on its own - neither Obsidian's nor this plugin's delete does that by default. Every delete this plugin makes - whether from your own action or one pulled in from another device - walks back up and removes each parent folder left with nothing in it. This is purely reactive (tied to an actual delete), on purpose: an earlier proactive version that swept the whole vault on every sync also caught brand-new folders that simply hadn't had a file added yet, deleting them out from under you. A folder that was already empty for some other reason (e.g. from before this plugin was installed) isn't touched - remove it by hand if you don't want it
- **Other plugins sync too, minus their secrets**: `.obsidian/community-plugins.json` and each installed plugin's `main.js`/`manifest.json`/`styles.css` are synced like any other file, so a new device gets every plugin already installed and enabled instead of reinstalling each one by hand. Every plugin's `data.json` is excluded on purpose, by filename rather than by plugin name - Obsidian has no secure-storage API for plugins, so anything needing an API key or token (this plugin included) has nowhere to put it except a plain-text data.json, and syncing that wholesale would leak it to the repo. This means plugin settings (templates folders, themes, etc.) still need setting up once per device - only the plugin code and enabled/disabled state travel with the sync
- **Automation**:
