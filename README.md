...
- **A new folder is never mistaken for an abandoned one**: git has no concept of an empty folder at all - one can only ever exist there because some file's path happens to be inside it. Every folder you create gets a small `_keep.md` placeholder the instant it's created, so it's never ambiguously empty in the first place (delete it once you've added real content). This sidesteps guessing "is this folder new or old" entirely, and means a brand-new empty folder syncs to your other devices just like any other file - no special-casing needed
