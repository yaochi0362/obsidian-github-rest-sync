...
- **Deletion tombstones**: a plain 3-way diff can't tell a genuinely new local file apart from an already-deleted one that came back (Obsidian's own undo, a stale wikilink auto-creating its target note) - it just looks like new content and gets pushed straight back up. Every deletion is logged to a day-sharded record (`.github-rest-sync-tombstones/YYYY-MM-DD.json`, invisible in the normal file list/report) that every device consults; a resurrected file is deleted again instead of re-uploaded. Records older than 10 days are pruned automatically on open
