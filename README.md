...
- **A new folder is never mistaken for an abandoned one**: git has no concept of an empty folder at all - one can only ever exist there because some file's path happens to be inside it. Every folder you create gets a small `.folderKeep` placeholder the instant it's created (dot-prefixed like git's own `.gitkeep` convention, so Obsidian's file explorer hides it), so it's never ambiguously empty in the first place - delete it once you've added real content. This sidesteps guessing "is this folder new or old" entirely, and means a brand-new empty folder syncs to your other devices just like any other file - no special-casing needed
- **No leftover empty folders**: deleting a file doesn't remove its now-empty parent folder on its own - neither Obsidian's nor this plugin's delete does that by default. Every sync proactively sweeps the whole vault removing any folder left with nothing in it (including one that only contains other now-empty folders), so this cleans up existing leftovers (e.g. from before this plugin, or from another sync tool) too, not just ones from a delete made after this feature shipped
